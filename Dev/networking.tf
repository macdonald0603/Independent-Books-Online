...
# resource "aws_acm_certificate" "ibo_dev_app_cert" {
#   domain_name       = "iboevent.com"
#   validation_method = "DNS"
#   tags = { Name = "ibo-dev-app-cert" }
# }
...
#   for_each = {
#     for dvo in aws_acm_certificate.ibo_dev_app_cert.domain_validation_options : dvo.domain_name => {
...
# resource "aws_acm_certificate_validation" "ibo_dev_app_cert_validation" {
#   certificate_arn         = aws_acm_certificate.ibo_dev_app_cert.arn
...
resource "aws_instance" "ibo_dev_app" {
  ...
  tags = { Name = "ibo-dev-app-server" }
}

# Load Balancer for HTTP and HTTPS traffic
resource "aws_lb" "ibo_dev_app_lb" {
  name               = "ibo-dev-app-lb"
  internal           = false
  ...
  tags = { Name = "ibo-dev-app-lb" }
}

# Target Group pointing to the EC2 instance
resource "aws_lb_target_group" "ibo_dev_app_tg" {
  name        = "ibo-dev-app-tg"
  port        = 80
  protocol    = "HTTP"
  ...
  tags = { Name = "ibo-dev-app-tg" }
}

# Register the EC2 instance with the Target Group
resource "aws_lb_target_group_attachment" "ibo_dev_app_tg_attachment" {
  target_group_arn = aws_lb_target_group.ibo_dev_app_tg.arn
  target_id        = aws_instance.ibo_dev_app.id
  port             = 80
}

# HTTP Listener to redirect traffic to HTTPS
resource "aws_lb_listener" "ibo_dev_app_http_listener" {
  load_balancer_arn = aws_lb.ibo_dev_app_lb.arn
  port              = 80
  protocol          = "HTTP"
...
# resource "aws_lb_listener" "ibo_dev_app_https_listener" {
#   load_balancer_arn = aws_lb.ibo_dev_app_lb.arn
#   port              = 443
#   protocol          = "HTTPS"
...
resource "aws_route53_record" "ibo_dev_app_record" {
  zone_id = aws_route53_zone.iboevent_zone.zone_id
  name    = "iboevent.com"
  type    = "A"
  alias {
    name                   = aws_lb.ibo_dev_app_lb.dns_name
    zone_id                = aws_lb.ibo_dev_app_lb.zone_id
