    ...
    Statement = [
      {
        ...
        Action    = "s3:PutObject",
        Resource  = "${aws_s3_bucket.ibo_prod_app_monitoring_bucket.arn}/AWSLogs/${data.aws_caller_identity.current.account_id}/*",
        Condition = {
          StringEquals = {
      ...
      },
      {
        ...
        Resource  = aws_s3_bucket.ibo_prod_app_monitoring_bucket.arn
      }
    ]
  })
}
...
resource "aws_cloudwatch_metric_alarm" "ibo_prod_app_ec2_cpu_alarm" {
  alarm_name          = "ibo-prod-app-ec2-cpu-usage-high"
  ...
  threshold           = 85
  dimensions = {
    InstanceId = aws_instance.ibo_prod_web_1.id # Make sure this EC2 instance is defined in the configuration
  }
