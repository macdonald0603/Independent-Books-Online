...
# resource "aws_acm_certificate" "ibo_prod_app_cert" {
#   domain_name       = "iboevent.com"
#   validation_method = "DNS"
#   tags = { Name = "ibo-prod-app-cert" }
# }
...
#   for_each = {
#     for dvo in aws_acm_certificate.ibo_prod_app_cert.domain_validation_options : dvo.domain_name => {
...
# resource "aws_acm_certificate_validation" "ibo_prod_app_cert_validation" {
#   certificate_arn         = aws_acm_certificate.ibo_prod_app_cert.arn
...
resource "aws_instance" "ibo_prod_app" {
  ...
  tags = { Name = "ibo-prod-app-server" }
}

# Load Balancer for HTTP and HTTPS traffic
resource "aws_lb" "ibo_prod_app_lb" {
  name               = "ibo-prod-app-lb"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.vpc_web_sg.id]
  subnets            = [aws_subnet.public_subnet_1.id, aws_subnet.public_subnet_2.id]

  tags = { Name = "ibo-prod-app-lb" }
}

# Target Group pointing to the EC2 instance
resource "aws_lb_target_group" "ibo_prod_app_tg" {
  ...
  tags = { Name = "ibo-prod-app-tg" }
}

# Register the EC2 instance with the Target Group
resource "aws_lb_target_group_attachment" "ibo_prod_app_tg_attachment" {
  target_group_arn = aws_lb_target_group.ibo_prod_app_tg.arn
  target_id        = aws_instance.ibo_prod_app.id
  port             = 80
}

# HTTP Listener to redirect traffic to HTTPS
resource "aws_lb_listener" "ibo_prod_app_http_listener" {
  load_balancer_arn = aws_lb.ibo_prod_app_lb.arn
  port              = 80
  protocol          = "HTTP"
...
# resource "aws_lb_listener" "ibo_prod_app_https_listener" {
#   load_balancer_arn = aws_lb.ibo_prod_app_lb.arn
#   port              = 443
#   protocol          = "HTTPS"
...
resource "aws_route53_record" "ibo_prod_app_record" {
  zone_id = aws_route53_zone.iboevent_zone.zone_id
  name    = "iboevent.com"
  type    = "A"
  alias {
    name                   = aws_lb.ibo_prod_app_lb.dns_name
    zone_id                = aws_lb.ibo_prod_app_lb.zone_id
